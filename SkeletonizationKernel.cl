...
__kernel void skeletonization3(__read_write image2d_t inputImage, __read_write image2d_t outputImage)
{
	int2 coord = (int2)(get_global_id(0), get_global_id(1));
...
__kernel void skeletonization_check(__read_write image2d_t inputImage, __read_write image2d_t outputImage)
{
	...
	float4 Gx = (float4)(0);
	float4 Gy = Gx;
	if(coord.x == 4322 && coord.y == 5557) printf("%d , %d , %d \n", read_imageui(inputImage,coord).x, read_imageui(inputImage,coord).y, read_imageui(inputImage,coord).z);
	//if(coord.x == 4322 && coord.y == 5557) write_imageui(outputImage, coord, convert_uint4((float4)(0)));
